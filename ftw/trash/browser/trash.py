...
class TrashView(BrowserView):
    ...
    clean_confirmation_tempalte = ViewPageTemplateFile(
        "templates/clean-trash-confirmation.pt"
    )
    ...
    def __call__(self):
        ...
        self.can_clean_trash = getSecurityManager().checkPermission(
            "Clean trash", self.context
        )
    ...
    def delete_permanently(self, REQUEST, uuid):
        ...
        with api.env.adopt_roles(["Manager"]):
            ...
            if got_path != path:
                raise ValueError(
                    "Unexpectly found path {!r} when looking for {!r}".format(
                        got_path, path
                    )
                )
            ...
            IStatusMessage(self.request).addStatusMessage(
                _(
                    u"statusmessage_content_permanently_deleted",
        ...
        if self.request.form.get("cancel"):
            ...
            with api.env.adopt_roles(["Manager"]):
                for path in paths_to_delete:
                    ...
                    aq_parent(aq_inner(obj)).manage_immediatelyDeleteObjects(
                        [obj.getId()]
                    )
